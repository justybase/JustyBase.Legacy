...
namespace FastColoredTextBoxNS
{
    ...
    public class ExportToHTML
    {
        ...
        public string GetHtml(FastColoredTextBox tb, bool forceDark = false)
        {
            this.tb = tb;
            Range sel = new Range(tb);
            sel.SelectAll();
            return GetHtml(sel, forceDark);
        }
        
        public string GetHtml(Range r, bool forceDark = false)
        {
            this.tb = r.tb;
            ...
            if (UseStyleTag)
            {
                ...
                foreach (var styleId in styles.Keys)
                {
                    ...
                    if (forceDark)
                    {
                        tempSB.AppendFormat(".fctb{0}{{ {1} }}\r\n", GetStyleName(styleId), "background-color: #000000;");
                    }
                    else
                    {
                        tempSB.AppendFormat(".fctb{0}{{ {1} }}\r\n", GetStyleName(styleId), "background-color: #FFFFFF;");
                    }
                }
                tempSB.Append("</style>");
