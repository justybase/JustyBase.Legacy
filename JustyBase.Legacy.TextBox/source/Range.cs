...
namespace FastColoredTextBoxNS
{
    ...
    public class Range : IEnumerable<Place>
    {
        ...
        public void GoRight(bool shift, bool dontCheckVisible = false)
        {
            ColumnSelectionMode = false;
            ...
            if (start.iLine < tb.LinesCount - 1 || start.iChar < tb[tb.LinesCount - 1].Count)
            {
                if (start.iChar < tb[start.iLine].Count && (dontCheckVisible || tb.LineInfos[start.iLine].VisibleState == VisibleState.Visible))
                    start.Offset(1, 0);
                else
                {
