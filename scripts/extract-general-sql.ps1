param(
    [string]$MethodName = "RunForGeneralEx",
    [string]$SourcePath = "$PSScriptRoot\..\JustData\BaseWindow.SqlExecution.cs",
    [string]$TargetPath = "$PSScriptRoot\..\JustData\BaseWindow.GeneralSqlEngine.cs"
)
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match "private async Task $MethodName\(") { $start = $i; break }
}
if ($start -lt 0) { throw "$MethodName not found" }
...
if ($end -lt 0) { throw "$MethodName end not found" }

$methodLines = $lines[$start..$end]
$header = @(
    '// General SQL execution core (Postgres, Oracle, DB2, etc.) extracted from BaseWindow.SqlExecution.cs.',
...
Write-Host "Wrote $MethodName ($($methodLines.Count) lines) to $TargetPath"
