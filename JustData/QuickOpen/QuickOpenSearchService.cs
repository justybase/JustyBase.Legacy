...
internal sealed class QuickOpenSearchService
{
    ...
    public QuickOpenSearchService(IFileSearchEngine fileSearchEngine)
    {
        ...
        IEnumerable<(EditorDocumentId Id, string Title, string? FilePath, string Text)> openDocuments)
    {
        ...
        void Upsert(string? filePath, EditorDocumentId? documentId, string displayName, string? inMemoryText, QuickOpenSource source)
        {
            string key = !string.IsNullOrWhiteSpace(filePath)
                ? "p:" + NormalizePath(filePath!)
                : "d:" + documentId!.Value.Value.ToString("N");
        ...
        foreach (var doc in openDocuments)
        {
            ...
            Upsert(
                ...
                doc.Id,
                string.IsNullOrWhiteSpace(doc.Title) ? "Untitled" : doc.Title,
                doc.Text,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var outcome = await _fileSearchEngine.SearchAsync(
                pathsToSearch,
                new FileSearchOptions
                {
                    Query = q,
                    ExtensionPatterns = SqlExtensions,
                    MaxFiles = MaxContentHits,
                    MaxMatchesPerFile = 3,
                    Timeout = timeout,
                },
            ...
            {
                if (!byPath.TryGetValue(NormalizePath(file.Path), out var candidate))
                {
        ...
        string query)
    {
        ...
        while ((line = reader.ReadLine()) is not null)
        {
            ...
            {
                int index = line.IndexOf(query, offset, StringComparison.OrdinalIgnoreCase);
                if (index < 0)
                    break;

                yield return (lineNumber, line, index, query.Length);
                offset = index + Math.Max(1, query.Length);
                // One match per line is enough for the Quick Open list.
                break;
            }
        }
    }
