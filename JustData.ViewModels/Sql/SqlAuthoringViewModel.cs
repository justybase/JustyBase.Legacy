...
public sealed class SqlAuthoringViewModel : ObservableObject, IDisposable
{
        ...
        TimeSpan? debounce = null)
    {
        ...
        try
        {
            TimeSpan delay = debounce ?? TimeSpan.FromMilliseconds(500);
            if (delay > TimeSpan.Zero)
                await Task.Delay(delay, cancellation.Token);

            await RunLintAsync(sqlText, connectionName, version, cancellation.Token);
        }
    ...
    public Task LintOnSaveAsync(string sqlText, string connectionName = "") =>
        LintOnSave
            ? LintNowAsync(sqlText, connectionName)
            : Task.CompletedTask;

    public async Task<SqlLintResult> LintNowAsync(string sqlText, string connectionName = "")
    {
        ThrowIfDisposed();
        CancellationTokenSource cancellation = RenewLintCancellation();
        long version = Interlocked.Increment(ref _lintVersion);
        try
        {
            return await RunLintAsync(sqlText, connectionName, version, cancellation.Token);
        }
        catch (OperationCanceledException) when (cancellation.IsCancellationRequested)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        SqlLintResult result = await _useCase.LintAsync(
            new SqlLintRequest(_documentId, sqlText ?? string.Empty, connectionName),
            cancellationToken);
