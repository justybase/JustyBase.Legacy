...
public sealed class AutocompleteSuggestionStoreTests
{
    ...
    {
        Assert.NotNull(AutocompleteSuggestionStore.Default);
        Assert.IsAssignableFrom<IAutocompleteSuggestionStore>(AutocompleteSuggestionStore.Default);
    }

    [Fact]
    public void Static_wrappers_delegate_to_Default()
    {
        // Save original
        var original = DynamicCollectionForGeneralHelpers.Default;

        try
        {
            var testStore = new AutocompleteSuggestionStore();
            testStore.OneWord.Add("test_item");
            DynamicCollectionForGeneralHelpers.Default = testStore;

            Assert.Single(DynamicCollectionForGeneralHelpers.OneWord);
            Assert.Equal("test_item", DynamicCollectionForGeneralHelpers.OneWord[0]);
        }
        finally
        {
            // Restore
            DynamicCollectionForGeneralHelpers.Default = original;
        }
    }

    [Fact]
    public void Mock_store_can_be_injected()
    {
        var mock = new MockAutocompleteStore();
        mock.OneWord.Add("mock_item");

        var original = DynamicCollectionForGeneralHelpers.Default;
        try
        {
            DynamicCollectionForGeneralHelpers.Default = mock;

            Assert.Equal("mock_item", DynamicCollectionForGeneralHelpers.OneWord[0]);
        }
        finally
        {
            DynamicCollectionForGeneralHelpers.Default = original;
    ...
    /// <summary>
    /// Simple mock implementing IAutocompleteSuggestionStore for testing.
    /// </summary>
    private sealed class MockAutocompleteStore : IAutocompleteSuggestionStore
    {
